...
#define MOKUKU_SERVICE_UUID "4fafc201-1fb5-459e-8fcc-c5c9c331914b"
#define MOKUKU_CHARACTERISTIC_UUID_MAIN "beb5483e-36e1-4688-b7f5-ea07361b26a8"
#define MOKUKU_CHARACTERISTIC_UUID_ACK "d222e154-1a80-4e71-9a63-2aa2c0ce0a8c"
...
#define CONNECT_RSSI_THRESHOLD -40

#define ESP_BLE_GAP_ADV_ITVL_MS(t)                                                                           \
  ((uint16_t)((t)*1000 / 625)) /*!< Convert advertising interval from ms to 0.625ms units. If input is not a \
                                  multiple of 0.625ms, it will be rounded to the nearest value. */
#define ESP_BLE_GAP_SCAN_ITVL_MS(t)                                                                   \
  ((uint16_t)((t)*1000 / 625)) /*!< Convert scan interval from ms to 0.625ms units. If input is not a \
                                  multiple of 0.625ms, it will be rounded to the nearest value. */
#define ESP_BLE_GAP_SCAN_WIN_MS(t)                                                                           \
  ((uint16_t)((t)*1000 / 625)) /*!< Convert scan window from ms to 0.625ms units. If input is not a multiple \
                                  of 0.625ms, it will be rounded to the nearest value. */
#define ESP_BLE_GAP_CONN_ITVL_MS(t)                                                                         \
  ((uint16_t)((t)*1000 / 1250)) /*!< Convert connection interval from ms to 1.25ms units. If input is not a \
                                   multiple of 1.25ms, it will be rounded to the nearest value. */
#define ESP_BLE_GAP_PERIODIC_ADV_ITVL_MS(t)                                                                  \
  ((uint16_t)((t)*1000 / 1250)) /*!< Convert periodic advertising interval from ms to 1.25ms units. If input \
                                   is not a multiple of 1.25ms, it will be rounded to the nearest value. */
#define ESP_BLE_GAP_SUPERVISION_TIMEOUT_MS(t)                                                      \
  ((uint16_t)((t) / 10)) /*!< Convert supervision timeout from ms to 10ms units. If input is not a \
                            multiple of 10ms, it will be rounded to the nearest value. */

static char remote_device_prefix[ESP_BLE_ADV_NAME_LEN_MAX] = "mokuku";
static bool connect = false;
static bool get_server = false;
static bool get_char = false;
...
static esp_bt_uuid_t remote_filter_service_uuid = {
    ...
    .uuid = {
        .uuid128 = {0x4b, 0x91, 0x31, 0xc3, 0xc9, 0xc5, 0xcc, 0x8f, 0x9e, 0x45, 0xb5, 0x1f, 0x01, 0xc2, 0xaf, 0x4f}}};

static esp_bt_uuid_t remote_filter_char_uuid = {.len = ESP_UUID_LEN_128,
                                                .uuid = {.uuid128 = {0x8c, 0x0a, 0xce, 0xc0, 0xa2, 0x2a, 0x63, 0x9a,
                                                                     0x71, 0x4e, 0x80, 0x1a, 0x54, 0xe1, 0x22, 0xd2}}};
...
static void log_char_props(esp_gatt_char_prop_t prop) {
  printf("%s Properties: 0x%02x [", GATTC_TAG, prop);
  if (prop & ESP_GATT_CHAR_PROP_BIT_READ) printf(" READ");
  if (prop & ESP_GATT_CHAR_PROP_BIT_WRITE) printf(" WRITE");
  if (prop & ESP_GATT_CHAR_PROP_BIT_NOTIFY) printf(" NOTIFY");
  if (prop & ESP_GATT_CHAR_PROP_BIT_INDICATE) printf(" INDICATE");
  if (prop & ESP_GATT_CHAR_PROP_BIT_WRITE_NR) printf(" WRITE_NR");
  if (prop & ESP_GATT_CHAR_PROP_BIT_BROADCAST) printf(" BROADCAST");
  printf(" ]\n");
}
  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
      ...
      if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_128 &&
          memcmp(p_data->search_res.srvc_id.uuid.uuid.uuid128, remote_filter_service_uuid.uuid.uuid128, 16) == 0) {
        get_server = true;
        gl_profile_tab[PROFILE_A_APP_ID].service_start_handle = p_data->search_res.start_handle;
        gl_profile_tab[PROFILE_A_APP_ID].service_end_handle = p_data->search_res.end_handle;
        uint8_t* u = p_data->search_res.srvc_id.uuid.uuid.uuid128;
        ESP_LOGI(GATTC_TAG,
                 "Service found with UUID128: "
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
      ...
      if (get_server) {
        ...
        if (count > 0) {
          ...
          if (!char_elem_result) {
            ...
            status = esp_ble_gattc_get_char_by_uuid(
                ...
                gl_profile_tab[PROFILE_A_APP_ID].service_end_handle, remote_filter_char_uuid, char_elem_result, &count);
            if (status != ESP_GATT_OK) {
            ...
            for (uint16_t i = 0; i < count; i++) {
              uint8_t* u = char_elem_result[i].uuid.uuid.uuid128;
              if (u == NULL) continue;
              ESP_LOGI(GATTC_TAG,
                       "Characteristic %d with UUID128: "
                       "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                       i, u[15], u[14], u[13], u[12], u[11], u[10], u[9], u[8], u[7], u[6], u[5], u[4], u[3], u[2],
                       u[1], u[0]);
              log_char_props(char_elem_result[i].properties);
              get_char = true;
              if (char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_WRITE_NR) {
                gl_profile_tab[PROFILE_A_APP_ID].char_handle = char_elem_result[0].char_handle;
              }
            }

            /*  Every service have only one char in our 'ESP_GATTS_DEMO' demo, so we used first
             * 'char_elem_result' */
        ...
        ESP_LOGE(GATTC_TAG, "no service found");
      }
      break;
      ...
      if (p_data->reg_for_notify.status != ESP_GATT_OK) {
        ...
        if (count > 0) {
          ...
          if (!descr_elem_result) {
            ...
            /* Every char has only one descriptor in our 'ESP_GATTS_DEMO' demo, so we used first
             * 'descr_elem_result' */
            if (count > 0 && descr_elem_result[0].uuid.len == ESP_UUID_LEN_16 &&
              ...
              ret_status = esp_ble_gattc_write_char_descr(
                  gattc_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id, descr_elem_result[0].handle, sizeof(notify_en),
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
      ...
      esp_ble_gattc_write_char(gattc_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                               gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(write_char_data), write_char_data,
...
// https://github.com/espressif/esp-idf/blob/release/v5.5/examples/bluetooth/bluedroid/ble/ble_multi_conn/ble_multi_conn_cent/main/peer_manager.c
bool traverse_send_peer(uint16_t len, uint8_t* value) {
  ...
  for (size_t i = 0; i < PROFILE_NUM; i++) {
    if (gl_profile_tab[i].conn_id != 0xFFFF && gl_profile_tab[i].char_handle != 0xFFFF) {
      ESP_LOGI(GATTC_TAG, "write: if=%d conn_id=%u handle=%u len=%u", gl_profile_tab[i].gattc_if,
               gl_profile_tab[i].conn_id, gl_profile_tab[i].char_handle, len);
      esp_err_t ret =
          esp_ble_gattc_write_char(gl_profile_tab[i].gattc_if, gl_profile_tab[i].conn_id, gl_profile_tab[i].char_handle,
                                   len, value, ESP_GATT_WRITE_TYPE_NO_RSP, ESP_GATT_AUTH_REQ_NONE);
      if (ret != ESP_OK) {
        ESP_LOGE(GATTC_TAG, "write failed! ret = %s (%d)", esp_err_to_name(ret), ret);
        all_ret = false;
      }
    }
  }
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param) {
  ...
  switch (event) {
      ...
      switch (scan_result->scan_rst.search_evt) {
        case ESP_GAP_SEARCH_INQ_RES_EVT:
          adv_name = esp_ble_resolve_adv_data_by_type(
              scan_result->scan_rst.ble_adv, scan_result->scan_rst.adv_data_len + scan_result->scan_rst.scan_rsp_len,
              ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);
          if (adv_name_len < strlen(remote_device_prefix) || adv_name == NULL) {
            break;
          }
          // ESP_LOGI(GATTC_TAG, "Scan result, device "ESP_BD_ADDR_STR", name len %u, rssi %d",
          // ESP_BD_ADDR_HEX(scan_result->scan_rst.bda), adv_name_len, scan_result->scan_rst.rssi);
          // ESP_LOG_BUFFER_CHAR(GATTC_TAG, adv_name, adv_name_len);

          if (strncmp((char*)adv_name, remote_device_prefix, strlen(remote_device_prefix)) == 0) {
            // Note: If there are multiple devices with the same device name, the device may connect
            // to an unintended one. It is recommended to change the default device name to ensure
            // it is unique.
            ESP_LOGI(GATTC_TAG, "Scan result, device " ESP_BD_ADDR_STR ", name len %u, rssi %d",
                     ESP_BD_ADDR_HEX(scan_result->scan_rst.bda), adv_name_len, scan_result->scan_rst.rssi);
            if (scan_result->scan_rst.rssi > CONNECT_RSSI_THRESHOLD && connect == false) {
              ESP_LOGI(GATTC_TAG, "Close device found %s", adv_name);
              connect = true;
    ...
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
      ESP_LOGI(GATTC_TAG, "Packet length update, status %d, rx %d, tx %d", param->pkt_data_length_cmpl.status,
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t* param) {
  ...
  do {
    ...
    for (idx = 0; idx < PROFILE_NUM; idx++) {
      if (gattc_if == ESP_GATT_IF_NONE || /* ESP_GATT_IF_NONE, not specify a certain gatt_if, need
